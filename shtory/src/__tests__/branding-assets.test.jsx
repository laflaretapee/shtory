...
test('app loads branding overrides for the logo asset', () => {
  ...
  const logoAsset = path.resolve(process.cwd(), 'public/media/brand/logo1.webp')

  expect(main).toContain("import './styles/branding.css'")
  expect(main).toContain("import { brandLogoPath } from './branding/logoData'")
  expect(main).toContain("document.querySelector('link[rel=\"icon\"]')")
  expect(main).toContain('brandLogoPath')
  ...
  expect(logoData).toContain("export const brandLogoPath = `${import.meta.env.BASE_URL}media/brand/logo1.webp`")
